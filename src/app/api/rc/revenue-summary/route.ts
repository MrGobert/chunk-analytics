...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${ANALYTICS_API_URL}/api/analytics/revenue-summary?days=${days}`,
      {
        headers: {
    ...
    return NextResponse.json({
      ...
      mrrSource: data.mrrSource ?? 'unavailable',
      attributedMrr: data.attributedMrr ?? 0,
      attributedSubscribers: data.attributedSubscribers ?? 0,
      pricedSubscribers: data.pricedSubscribers ?? 0,
      unpricedSubscribers: data.unpricedSubscribers ?? 0,
      excludedNoProvenance: data.excludedNoProvenance ?? 0,
      excludedNonPaying: data.excludedNonPaying ?? 0,
      excludedFreeAccess: data.excludedFreeAccess ?? 0,
      excludedLapsed: data.excludedLapsed ?? 0,
      excludedNonUsd: data.excludedNonUsd ?? 0,
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      ...
      return NextResponse.json({
        ...
        byPlatform: {}, byProduct: {}, subscribersByProduct: {},
        pricedSubscribers: 0, unpricedSubscribers: 0, excludedNoProvenance: 0,
        mrrSource: 'unavailable', attributedMrr: 0, attributedSubscribers: 0,
        excludedNonPaying: 0, excludedFreeAccess: 0, excludedLapsed: 0,
        excludedNonUsd: 0, mrrTrend: [],
