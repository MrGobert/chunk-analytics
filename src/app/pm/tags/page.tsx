    ...
    return (
        <div className="animate-in fade-in duration-300 max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-serif text-ink mb-2">Labels & Tags</h1>
                    <p className="text-ink-soft font-mono text-sm tracking-tight">Organize tickets with customizable labels</p>
                </div>
            ...
            {(isCreating || editingId) && (
                <div className="bg-card border border-line rounded-2xl p-6 mb-8 shadow-xl">
                    <h2 className="text-lg font-bold text-ink mb-4">{editingId ? 'Edit Tag' : 'Create New Tag'}</h2>
                    <form onSubmit={editingId ? handleUpdate : handleCreate} className="space-y-6">
                        <div>
                            <label className="block text-xs font-semibold text-ink-soft uppercase tracking-wider mb-2">Tag Name</label>
                            <input
                                autoFocus
                                value={name}
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-ink-soft uppercase tracking-wider mb-3">Color</label>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-line">
                            <div className="flex items-center gap-3">
                                <span className="text-sm text-ink-faint">Preview:</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {tags.length === 0 && !isCreating ? (
                    ...
                    tags.map(t => (
                        <div key={t.id} className="bg-card/50 border border-line hover:border-line rounded-xl p-4 flex items-center justify-between group transition-colors">
                            <div className="flex items-center gap-3">
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => startEdit(t)} className="p-1.5 text-ink-soft hover:text-ink hover:bg-paper-deep rounded-lg">
                                    <Edit2 className="w-4 h-4" />
                                </button>
                                <button onClick={() => deleteTag(t.id)} className="p-1.5 text-ember-deep hover:text-ink hover:bg-red-500/20 rounded-lg">
