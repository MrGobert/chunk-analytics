...
export default function RevenuePage() {
  ...
  return (
    <div ref={containerRef} className="animate-in fade-in duration-300">
      ...
      {revenue && !fromRevenueCat && (
        <div className="mb-6 p-4 bg-butter-tint border border-butter rounded-card flex items-center gap-2 text-sm text-ink">
          <AlertTriangle className="w-5 h-5 text-[#C8922A] shrink-0" />
          <span>
            Showing revenue we can attribute from subscription records, not RevenueCat&apos;s own
            total — these figures understate actual revenue. Subscribers whose purchase never
            reached our webhook are missing entirely.
          </span>
        </div>
      )}
